  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/repository/permohonan", {
        ...
        // Kirim formData apa adanya — mapping dilakukan di route.ts
        body: JSON.stringify(formData),
      });

      if (!res.ok) {
        const err = await res.json();
        throw new Error(err.message ?? "Gagal mengirim permohonan");
      }
      ...
      toast({
        ...
        description: result.no_tiket
          ? `No. Tiket: ${result.no_tiket}`
                      ...
                      <tbody className="divide-y divide-slate-200/60 dark:divide-slate-700/60">
                        {[
                          ...
                          },
                          {
                            ...
                            name: "modulLainnya",
                            placeholder:
                              "Modul atau teknologi tambahan lainnya",
                          },
                        ].map((row) => (
                          <tr
                            key={row.name}
                  ...
                  <CheckboxButton
                    ...
                    onToggle={() =>
                      setFormData((prev) => ({
                        ...prev,
                        jenisDomain:
                          prev.jenisDomain === "perubahan" ? "" : "perubahan",
                      }))
                    }
                    label="Perubahan"
                  />
              ...
              <div className="overflow-hidden rounded-xl border-2 border-slate-200/80 dark:border-slate-700/80 shadow-sm">
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead>
                      <tr className="bg-gradient-to-r from-slate-100 to-slate-50 dark:from-slate-800 dark:to-slate-800/50 border-b-2 border-slate-200/80 dark:border-slate-700/80">
                        <th className="px-4 py-4 text-center text-xs font-semibold text-slate-900 dark:text-white w-12">
                          No.
                        </th>
                        <th className="px-4 py-4 text-left text-xs font-semibold text-slate-900 dark:text-white">
                          Nama Personil
                        </th>
                        <th className="px-4 py-4 text-left text-xs font-semibold text-slate-900 dark:text-white italic">
                          Username Gitlab
                        </th>
                        <th className="px-4 py-4 text-left text-xs font-semibold text-slate-900 dark:text-white">
                          Jabatan/Peran
                    ...
                    <tbody className="divide-y divide-slate-200/60 dark:divide-slate-700/60">
                      {formData.personil.map((row, index) => (
                        <tr
                          ...
                          className="bg-white dark:bg-slate-900/50 hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors"
                        >
                          <td className="px-4 py-3 text-sm font-semibold text-slate-500 dark:text-slate-400 text-center">
                            {index + 1}
                          </td>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 space-y-1">
              ...
              <p className="text-sm leading-relaxed text-slate-700 dark:text-slate-400">
                ...
                sesuai. Data yang sudah disubmit akan langsung diproses oleh tim
                pengelola SPBE.
              </p>
            </div>
          </div>
